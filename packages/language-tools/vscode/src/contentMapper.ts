...
function resolveMapperManifest(context: vscode.ExtensionContext): ContentMapperManifest {
	for (const folder of vscode.workspace.workspaceFolders ?? []) {
		try {
			const require = createRequire(path.join(folder.uri.fsPath, 'package.json'));
			const manifest = readManifest(require.resolve(`${MAPPER_PACKAGE}/package.json`));
	...
	return {
		name: MAPPER_PACKAGE,
		exec: [process.execPath, context.asAbsolutePath('dist/contentMapper/server.js')],
...
export async function registerContentMapper(context: vscode.ExtensionContext): Promise<boolean> {
	...
	const manifest = resolveMapperManifest(context);
	...
	try {
		context.subscriptions.push(
			api.registerContentMappers(CONTRIBUTOR_ID, [
				{
					extensions: ['.astro'],
					// Configured projects still use their tsconfig entry; this covers every other Astro file.
					inferredProjectContribution: { options: {}, manifest },
				},
			]),
		);
	} catch (error) {
