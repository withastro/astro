...
function renderSimpleChecks(html: string) {
	...
	const spanInsideH2 = document.querySelector<HTMLElement>('h2 > span')!;
...
function renderNestedHTMLChecks(html: string) {
	...
	const p1Span1 = p1.querySelector<HTMLElement>('span')!;
...
function renderRandomlyCasedHTMLAttributesChecks(html: string) {
	const { document } = parseHTML(html);
...
function renderHTMLWithinPartialChecks(html: string) {
	const { document } = parseHTML(html);
...
function renderComponentsHTMLChecks(html: string) {
	...
	assert.equal(p1.children.length, 1);
	assertInlineMark(p1.children[0] as HTMLElement);
	...
	assertInlineMark(div1p1.children[0] as HTMLElement);
	...
	assertInlineMark(div1p2span1.children[0] as HTMLElement);
	...
	assertInlineMark(section1Aside1SectionP4.children[0] as HTMLElement);
...
function assertInlineMark(el: HTMLElement | null | undefined) {
	assert.ok(el);
