...
async function getContentEntryType({ markdocConfigResult, astroConfig, options }) {
	return {
		extensions: ['.mdoc'],
...
function raiseValidationErrors({ ast, markdocConfig, viteId, astroConfig, filePath }) {
	const validationErrors = Markdoc.validate(ast, markdocConfig).filter((e) => {
		return (
			(e.error.level === 'error' || e.error.level === 'critical') && // Ignore `variable-undefined` errors.
			...
			if (shouldOptimizeImage(node.attributes.src)) {
				...
				if (resolved?.id && fs.existsSync(new URL(prependForwardSlash(resolved.id), 'file://'))) {
					const fileEmitter = ctx.pluginContext.meta.watchMode
						? void 0
						: (opts) => emitClientAsset(ctx.pluginContext, opts);
