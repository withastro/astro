...
describe('MDX plugins - Astro config integration', () => {
	...
	it('does not inherit global processor when extendMarkdownConfig is false', async () => {
		const fixture = await buildFixture({
			...
			markdown: {
				processor: satteri({
					hastPlugins: [
						{
							name: 'test-add-data-attr',
							element: {
								filter: ['p'],
								visit(node: any, ctx: any) {
									ctx.setProperty(node, 'data-hast-plugin', 'ok');
								},
							},
						},
					],
		...
		assert.equal(
			document.querySelector('p[data-hast-plugin]'),
			null,
			'Global processor hastPlugins should not be applied to MDX when extendMarkdownConfig is false',
		);
	});
});
