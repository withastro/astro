...
describe('SVG Deduplication', () => {
	...
	describe('build', () => {
		...
		it('deduplicates identical SVG files in build output', async () => {
			...
			assert.equal(svgFiles.length, 2, `Expected 2 unique SVG files, found ${svgFiles.length}: ${svgFiles.join(', ')}`);
		});
		...
		it('preserves all SVG URLs in HTML output', async () => {
			...
			assert.ok(duplicate1Svg.find('circle').length > 0, 'duplicate1 should contain circle element');
			assert.ok(duplicate2Svg.find('circle').length > 0, 'duplicate2 should contain circle element');
