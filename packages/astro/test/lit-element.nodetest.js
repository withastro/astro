...
describe('LitElement test', { timeout: 300000 }, () => {
	let fixture;
	...
	it('Correctly passes child slots', async () => {
		...
		assert.equal($slottedMyElement.children('.default').eq(0).text(), 'slotted my-element default');
		...
		assert.equal($slottedMyElement.children('[slot="named"]').eq(2).attr('id'), 'slotted-slotted');
