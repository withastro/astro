...
import type { AstroComponentMetadata } from '../dist/types/public/internal.js';
import { serializeProps } from '../dist/runtime/server/serialize.js';

const noMetadata: AstroComponentMetadata = { displayName: '', astroStaticSlot: true };

describe('serialize', () => {
	it('serializes undefined', () => {
		const input = { a: undefined };
		const output = `{"a":[0]}`;
		assert.equal(serializeProps(input, noMetadata), output);
	});
	it('serializes null', () => {
		const input = { a: null };
	...
	it('cannot serialize a cyclic reference', () => {
		...
		a.b = a;
		const input = { a };
		assert.throws(() => serializeProps(input, noMetadata), { message: /cyclic/ });
	});
	it('cannot serialize a cyclic array', () => {
		const input: { foo: unknown[] } = { foo: ['bar'] };
		input.foo.push(input);
		assert.throws(() => serializeProps(input, noMetadata), { message: /cyclic/ });
	});
	it('cannot serialize a deep cyclic reference', () => {
		const a: Record<string, Record<string, unknown>> = { b: {} };
		a.b.c = a;
		const input = { a };
		assert.throws(() => serializeProps(input, noMetadata), { message: /cyclic/ });
	});
	it('can serialize shared references that are not cyclic', () => {
		const b = {};
		const input = { a: { b }, b };
		assert.doesNotThrow(() => serializeProps(input, noMetadata));
	});
	...
	it('serializes a mixed-type array', () => {
		...
		assert.equal(serializeProps({ a: [1, 'hello', date] }, noMetadata), output);
	});
	it('includes component displayName and hydrate in cyclic error message', () => {
		const a: Record<string, unknown> = {};
		a.b = a;
		const metadata: AstroComponentMetadata = { displayName: 'MyComponent', hydrate: 'load', astroStaticSlot: true };
		assert.throws(
