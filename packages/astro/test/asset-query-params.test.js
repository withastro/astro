...
describe('Asset Query Parameters in Inter-Chunk JS Imports', () => {
	...
	it('appends assetQueryParams to relative imports inside client JS chunks', async () => {
		...
		assert.ok(
			foundStaticImport,
			'Expected at least one relative static inter-chunk import in client JS files',
		);
		assert.ok(
			foundDynamicImport,
			'Expected at least one relative dynamic inter-chunk import in client JS files',
		);
	});
});
