		...
		describe('deploy config provided', () => {
			...
			before(async () => {
				fixture = await loadFixture({
					...
					cacheDir: './node_modules/.astro-test/config-mode-deploy-config-provided/',
				});
				await fixture.build();
			});
		...
		describe('deploy config omitted', () => {
			...
			before(async () => {
				fixture = await loadFixture({
					...
					cacheDir: './node_modules/.astro-test/config-mode-deploy-config-omitted/',
				});
			});
			...
			it('Throws during the build', async () => {
				...
				try {
					...
					assert.match((err as Error).message, /without an adapter/);
				}
		...
		describe('Output config omitted', () => {
			...
			before(async () => {
				fixture = await loadFixture({
					...
					cacheDir: './node_modules/.astro-test/config-mode-output-config-omitted/',
				});
				await fixture.build();
			});
