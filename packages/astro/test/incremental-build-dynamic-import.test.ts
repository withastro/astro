...
describe('experimental.incrementalBuild dynamic imports', () => {
	const root = new URL('./fixtures/incremental-build-dynamic-import/', import.meta.url);
	const cacheFile = new URL('node_modules/.astro/incremental-build.json', root);
	const depFile = new URL('src/lib/dep.js', root);
	...
	before(async () => {
		...
		// Change a module the page reaches only through `await import(...)`.
	...
	it('records a dependency hash for a route with a dynamic import', () => {
		assert.ok(hashBefore, 'route should be tracked in the cache');
	});

	it('changes the route dependency hash when a dynamically imported dependency changes', () => {
