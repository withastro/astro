	...
	it('should correctly return the URL with the base', () => {
		...
		assert.equal(
			getLocaleRelativeUrl({
				locale: 'en',
				base: '/blog/',
				trailingSlash: { page: 'always', endpoint: 'always' },
	...
	it('should correctly handle the trailing slash', () => {
		...
		assert.equal(
			getLocaleRelativeUrl({
				...
				base: '/blog',
				...config.i18n,
				trailingSlash: { page: 'never', endpoint: 'never' },
			}),
			'/blog',
		);
		assert.equal(
		...
		assert.equal(
			getLocaleRelativeUrl({
				...
				base: '/blog/',
				...config.i18n,
				trailingSlash: { page: 'ignore', endpoint: 'ignore' },
			}),
			'/blog/',
		);
...
describe('getLocaleAbsoluteUrlList', () => {
	...
	it('should retrieve the correct list of base URLs, swapped with the correct domain', () => {
		...
		assert.deepEqual(
			...
			['https://example.uk/blog', 'https://example.com/blog/en-us', 'https://es.example.com/blog'],
		);
	});
});
