...
import type { TextNode } from '../../../dist/runtime/server/render/queue/types.js';
...
describe('Queue-based rendering engine', () => {
	...
	describe('buildRenderQueue()', () => {
		it('should handle simple text nodes', async () => {
			...
			assert.equal((queue.nodes[0] as TextNode).content, 'Hello, World!');
		});
		...
		it('should handle numbers', async () => {
			...
			assert.ok(queue.nodes.length > 0);
			assert.equal(queue.nodes[0].type, 'text');
			assert.equal((queue.nodes[0] as TextNode).content, '42');
		});
		...
		it('should handle booleans', async () => {
			...
			assert.ok(queue.nodes.length > 0);
			assert.equal(queue.nodes[0].type, 'text');
			assert.equal((queue.nodes[0] as TextNode).content, 'true');
		});
		...
		it('should handle arrays', async () => {
			...
			assert.equal((queue.nodes[0] as TextNode).content, 'Hello');
			assert.equal((queue.nodes[1] as TextNode).content, ' ');
			assert.equal((queue.nodes[2] as TextNode).content, 'World');
		});
		...
		it('should skip false but render 0', async () => {
			...
			assert.equal((zeroQueue.nodes[0] as TextNode).content, '0');
		});
		...
		it('should handle promises', async () => {
			...
			assert.equal((queue.nodes[0] as TextNode).content, 'Resolved value');
		});
		...
		it('should handle nested arrays', async () => {
			...
			assert.equal(queue.nodes.length, 3);
			assert.equal((queue.nodes[0] as TextNode).content, 'Nested');
			assert.equal((queue.nodes[1] as TextNode).content, ' ');
			assert.equal((queue.nodes[2] as TextNode).content, 'Array');
		});
		...
		it('should handle async iterables', async () => {
			...
			assert.equal(queue.nodes.length, 3);
			assert.equal((queue.nodes[0] as TextNode).content, 'First');
			assert.equal((queue.nodes[1] as TextNode).content, 'Second');
			assert.equal((queue.nodes[2] as TextNode).content, 'Third');
		});
		...
		it('should track parent relationships', async () => {
			...
			assert.equal(queue.nodes.length, 3);
			assert.equal((queue.nodes[0] as TextNode).content, 'child1');
			assert.equal((queue.nodes[1] as TextNode).content, 'child2');
			assert.equal((queue.nodes[2] as TextNode).content, 'sibling');
		});
		...
		it('should maintain correct rendering order', async () => {
			...
			assert.equal((queue.nodes[0] as TextNode).content, 'A');
			assert.equal((queue.nodes[1] as TextNode).content, 'B');
			assert.equal((queue.nodes[2] as TextNode).content, 'C');
		});
		...
		it('should handle sync iterables (Set)', async () => {
			...
			const contents = queue.nodes.map((n) => (n as TextNode).content);
...
describe('renderPage() with queuedRendering and .html pages', () => {
	...
	it('does not escape HTML tags when rendering a .html page component', async () => {
		...
		(htmlPageFactory as unknown as Record<string, unknown>)['astro:html'] = true;
		(htmlPageFactory as unknown as Record<string, unknown>).moduleId = 'src/pages/admin/index.html';

		const result = createMockResultWithQueue();

		const response = await renderPage(result, htmlPageFactory as any, {}, null, false);
	...
	it('still escapes HTML in non-.html page components with queuedRendering', async () => {
		...
		(regularFactory as unknown as Record<string, unknown>).moduleId = 'src/pages/regular.astro';

		const result = createMockResultWithQueue();

		const response = await renderPage(result, regularFactory as any, {}, null, false);
		const html = await response.text();
