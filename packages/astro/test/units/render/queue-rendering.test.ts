...
describe('renderPage() with queuedRendering and .html pages', () => {
	...
	it('does not escape HTML tags when rendering a .html page component', async () => {
		...
		const htmlPageFactory: AstroComponentFactory = function render(_props: unknown) {
			return '<body>\n  <script src="https://unpkg.com/@sveltia/cms/dist/sveltia-cms.js"></script>\n</body>' as any;
		};
		// `astro:html` is an internal runtime property set by vite-plugin-html, not part of the type
	...
	it('still escapes HTML in non-.html page components with queuedRendering', async () => {
		...
		const regularFactory: AstroComponentFactory = function render(_props: unknown) {
			return '<script>alert("xss")</script>' as any;
		};
