...
describe('NodePool', () => {
	...
	it('should reuse released nodes of the same type', () => {
		...
		const node2 = pool.acquire('text') as TextNode;
		strictEqual(node2.type, 'text');
		strictEqual(node2.content, ''); // Content was reset on release
		strictEqual(node1 as unknown, node2 as unknown); // Same object reference (actual reuse)
	...
	it('should properly create nodes with correct discriminated union types', () => {
		...
		const textNode = pool.acquire('text') as TextNode;
		const htmlNode = pool.acquire('html-string') as HtmlStringNode;
		const componentNode = pool.acquire('component') as ComponentNode;
		const instructionNode = pool.acquire('instruction') as InstructionNode;
	...
	it('should return the same object reference when reusing pooled nodes', () => {
		...
		for (const type of types) {
			...
			strictEqual(
				original as unknown,
				reused as unknown,
				`${type} node should be same object after reuse`,
			);
		}
	});
	...
	it('should clear references on component and instruction nodes when released', () => {
		...
		compNode.instance = { render: () => {} } as unknown as ComponentNode['instance']; // Simulate a component instance
		pool.release(compNode);

		const reusedComp = pool.acquire('component') as ComponentNode;
		strictEqual(reusedComp as unknown, compNode as unknown); // Same object
		...
		strictEqual(reusedInstr as unknown, instrNode as unknown); // Same object
	...
	it('should track pool size across mixed types correctly', () => {
		...
		notStrictEqual(newText as unknown, text1 as unknown); // Not reused - new object
		notStrictEqual(newText as unknown, text2 as unknown); // Not reused - new object
	});
	...
	it('should apply shared maxSize cap across all sub-pools', () => {
		...
		notStrictEqual(newInstr as unknown, instr1 as unknown);
	});
	...
	it('should set content on reused nodes via acquire', () => {
		...
		strictEqual(reused as unknown, node as unknown); // Same object
	...
	it('should clear all sub-pools on clear()', () => {
		...
		const newText = pool.acquire('text');
		notStrictEqual(newText as unknown, nodes[0] as unknown);
	});
});
