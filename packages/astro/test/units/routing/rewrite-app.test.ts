...
describe('Rewrites via App - middleware sequence with next() vs context.rewrite()', () => {
	...
	const second: MiddlewareHandler = async (context, next) => {
		if (context.url.pathname.includes('/auth')) {
			if (context.url.pathname.includes('/auth/dashboard')) {
				contextReroute = true;
				return await context.rewrite('/');
			}
			if (context.url.pathname.includes('/auth/base')) return await next('/');
			if (context.url.pathname.includes('/auth/params')) return next('/?foo=bar');
			if (context.url.pathname.includes('/auth/astro-params')) return next('/auth/1234');
		}
		return next();
	};

	const third: MiddlewareHandler = async (context, next) => {
		if (context.url.pathname.startsWith('/') && contextReroute === false) {
			context.locals.auth = 'Third function called';
		}
		if ((context.params as any)?.id === '1234') {
...
describe('Rewrites via App - body-used rewrite returns 500', () => {
	const bodyUsedPage = createComponent(async (result, props, slots) => {
		const Astro = result.createAstro(props, slots);
		if (Astro.request.method === 'POST') await Astro.request.text();
	...
	it('returns 500 when rewriting after body has been consumed', async () => {
		...
		const res = await app.render(
			new Request('http://example.com/post/post-body-used', { method: 'POST', body: formData }),
		);
		assert.equal(res.status, 500);
	});
});
...
describe('Rewrites via App - routePattern updated after sequence rewrite', () => {
	...
	const first: MiddlewareHandler = async (context, next) => {
		if (context.url.pathname === '/index2') return next('/123/post');
