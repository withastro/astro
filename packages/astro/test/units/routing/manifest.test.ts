...
function getLogger() {
	const logs: Array<{ label: string; level: string; message: string; newLine: boolean }> = [];
...
describe('routing - createRoutesList', () => {
	it('using trailingSlash: "never" does not match the index route when it contains a trailing slash', async () => {
		const fixture = await createFixture({ '/src/pages/index.astro': `<h1>test</h1>` });
	...
	it('redirects are sorted alongside the filesystem routes', async () => {
		...
		const settings = await createBasicSettings({
			...
			trailingSlash: 'never',
			redirects: {
				'/blog/[...slug]': { status: 302, destination: '/' },
		...
		assert.deepEqual(getManifestRoutes(manifest), [
			...
			{ route: '/blog/contributing', type: 'page' },
			{ route: '/blog/[...slug]', type: 'redirect' },
			{ route: '/', type: 'page' },
		]);
	});
