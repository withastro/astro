...
describe('routing priority (unit)', () => {
	const router = new Router(routes, { base: '/', trailingSlash, buildFormat: 'directory' });

	it('matches / to index route', () => {
		const match = router.match('/');
		assert.equal(match.type, 'match');
		if (match.type === 'match') assert.equal(match.route.component, 'index.astro');
	});

	it('matches static locale route before dynamic', () => {
		const match = router.match('/de');
		assert.equal(match.type, 'match');
		if (match.type === 'match') assert.equal(match.route.component, 'de/index.astro');
	});
	...
	it('matches injected static routes', () => {
		...
		if (injected.type === 'match') assert.equal(injected.route.component, 'to-inject.astro');

		const underscored = router.match('/_injected');
		assert.equal(underscored.type, 'match');
		if (underscored.type === 'match') assert.equal(underscored.route.component, 'to-inject.astro');
	});
