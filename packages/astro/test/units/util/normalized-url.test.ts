...
describe('normalizeUrl', () => {
	...
	// A decoded backslash only becomes a duplicate slash once it has been
	// assigned back, because the pathname setter rewrites `\` to `/`:
	// `/a%5C/b` -> decode `/a\/b` -> assign `/a//b` -> collapse `/a/b`.
	// Collapsing the decoded string before assigning it would leave `/a//b`,
	// so this pins the order the two steps have to run in.
