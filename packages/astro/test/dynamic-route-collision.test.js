...
describe('Dynamic route collision', () => {
	...
	it('Builds a static route when in conflict with a rest route', async () => {
		const html = await fixture.readFile('/who/index.html');
		const $ = cheerio.load(html);
		assert.equal($('h1').text(), 'Static Who We Are');
	});

	it('Builds a static nested index when in conflict with a rest route', async () => {
		const html = await fixture.readFile('/tags/index.html');
		const $ = cheerio.load(html);
		assert.equal($('h1').text(), 'Static Tags Index');
	});

	it('Builds a static root index when in conflict with a rest route', async () => {
		const html = await fixture.readFile('/index.html');
		const $ = cheerio.load(html);
		assert.equal($('h1').text(), 'Static Index');
	});

	it('Builds a dynamic route when in conflict with a rest route', async () => {
