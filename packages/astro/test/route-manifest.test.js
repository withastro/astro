...
test('encodes invalid characters', () => {
	...
	assert.equal(routes.map((p) => p.pattern),
		[
			// /^\/%22$/,
			/^\/%23$/
			// /^\/%3F$/
...
test('ignores files and directories with leading dots except .well-known', () => {
	...
	assert.equal(routes.map((r) =>r.component).filter(Boolean), [
		'hidden-dot/.well-known/dnt-policy.astro'
	]);
});
...
test('disallows rest parameters inside segments', () => {
	assert.throws(
		...
		(e) => {
			return (
				e.message ===
				'Invalid route invalid-rest/foo-[...rest]-bar.astro — rest parameter must be a standalone segment'
			);
		}
	);
});
