...
/**
 * @param {string} href
 * @param {import('./test-utils').Fixture} f
 * @returns {Promise<{ href: string; css: string; }>}
 */
async function getLinkContent(href, f) {
	const css = await f.readFile(href);
	return { href, css };
}

describe('Mega frameworks fixture (default config)', () => {
	...
	describe('build', () => {
		...
		describe('Aliases without baseUrl (build)', () => {
	...
	describe('dev', () => {
		...
		describe('Aliases without baseUrl (dev)', () => {
			it('can resolve paths without baseUrl', async () => {
...
describe('Mega frameworks fixture (inlineStylesheets: never)', () => {
	/** @type {import('./test-utils').Fixture} */
	let fixture;
