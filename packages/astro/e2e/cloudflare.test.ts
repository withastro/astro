...
function sharedTests(testRunner: AstroTest, infoLogs: LogEntry[] | null = null) {
	...
	if (infoLogs) {
		testRunner('dev server logs', () => {
			const serverStartLog = infoLogs.find(
				(log) => typeof log.message === 'string' && log.message.includes('Local'),
			);
		...
		testRunner('all dependencies pre-optimized, none discovered', () => {
			...
			const optimizedLog = infoLogs.find(
				(log) =>
					typeof log.message === 'string' && log.message.includes('new dependencies optimized'),
			);
