...
function collectPreloads(page: Page) {
	return page.evaluate(() => {
		window.preloads = [];
		const observer = new MutationObserver((mutations) => {
			mutations.forEach((mutation) =>
				mutation.addedNodes.forEach((node) => {
					...
					if (link.nodeName === 'LINK' && link.rel === 'preload') window.preloads.push(link.href);
				}),
			);
		});
...
test.describe('View Transitions', () => {
	...
	test('<video> can persist using transition:persist', async ({ page, astro, browserName }) => {
		...
		await vid.evaluate((el) => ((el as HTMLVideoElement).muted = true));
		...
		await vid.click();
		await vid.evaluate((el) => (el as HTMLVideoElement).play());
	...
	test('Vite style nodes refresh generated CSS through head swaps', async ({ page, astro }) => {
		const expectLoads = collectLoads(page);
		await page.goto(astro.resolveUrl('/island-svelte-one'));

		const generatedStyle = page.locator('#generated-style');
		await expect(generatedStyle).toHaveCSS('color', 'rgb(255, 0, 0)');
		await page
			.locator('style[data-vite-dev-id="/__uno.css"]')
			.evaluate((element) => (element.dataset.marker = 'this'));

		await page.click('#click-two');
		await expect(generatedStyle).toHaveCSS('color', 'rgb(0, 0, 255)');
		await expect(
			page.locator('style[data-vite-dev-id="/__uno.css"][data-marker="this"]'),
		).toHaveCount(1);
		await expectLoads(1);
	});
	...
	test('client:only styles are retained on transition (2/2)', async ({ page, astro }) => {
		...
		styles = await page.locator('style').all();
		expect(styles.length, 'style count has not changed').toEqual(totalExpectedStyles_page_four);
	});
