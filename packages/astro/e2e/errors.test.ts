...
test.describe('Error display', () => {
	...
	test('shows correct line when a style preprocess has an error', async ({ page, astro }) => {
		...
		const lineNumber = Number(/:(\d+):\d+$/.exec(absoluteFileLocation!)![1]);
	...
	test('properly highlight the line with the error', async ({ page, astro }) => {
		...
		const codeFrame = (await getErrorOverlayContent(page)).codeFrame!;
	...
	test('copy stack trace to clipboard', async ({ page, astro, context }) => {
		...
		const copyButton = (await getErrorOverlayContent(page)).copyButton!;
