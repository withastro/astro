...
import { AstroError, AstroUserError, CompilerError } from '../errors/index.js';
...
function serverStart({ startupTime, resolvedUrls, host, base, astroVersionProvider, textStyler }) {
...
function printHelp({ commandName, headline, usage, tables, description }) {
	...
	if (headline) {
		message.push(
			linebreak(),
			`  ${bgGreen(black(` ${commandName} `))} ${green(`v${'6.3.3'}`)} ${headline}`,
		);
	}
	if (usage) {
