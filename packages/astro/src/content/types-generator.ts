...
async function writeContentFiles({
	...
	let configPathRelativeToCacheDir = normalizePath(
		path.relative(fileURLToPath(contentPaths.dotAstroDir), fileURLToPath(contentPaths.config.url))
	);
	...
	await fs.promises.writeFile(contentPaths.dotAstroTypes, typeTemplateContent);
}
