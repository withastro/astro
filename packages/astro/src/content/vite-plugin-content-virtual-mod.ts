...
export function astroContentVirtualModPlugin({
	...
	return {
		...
		resolveId: {
			filter: {
				id: new RegExp(
					`^(${VIRTUAL_MODULE_ID}|${DATA_STORE_VIRTUAL_ID}|${MODULES_MJS_ID}|${ASSET_IMPORTS_VIRTUAL_ID})$|(\\?${CONTENT_MODULE_FLAG}=)|(&${CONTENT_MODULE_FLAG}=)|(\\?${CONTENT_MODULE_FLAG}$)|(&${CONTENT_MODULE_FLAG}$)`,
				),
			},
