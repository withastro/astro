...
export interface AstroUserConfig {
	...
	experimental?: {
		 ...
		 * @description
		 *
		 * Enables a streamlined way of configuring TypeScript in your project for better DX when setting up Astro for Intellisense, type-checking, and more.
		 *
		 * This option removes the need for a separate `src/env.d.ts` file to provide Vite's client types to your project. Instead, `/.astro/tsconfig.json` will be autogenerated with linked types for you. Notably, this fixes an existing issue with `astro check` false positives by excluding `outDir` by default.
		 *
		 * To enable this option in an existing project, you will need to update your existing `tsconfig.json` to reference the new file, and also potentially your `astro.config.mjs` as some items have now moved to Astro configuration options.
		 *
		 * Your `tsconfig.json` file must now extend at least the new `/.astro/` file:
		 *
