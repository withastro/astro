...
/**
 * Regex matching CSS @import and @reference statements with the specifier in capture group 1.
 * Handles @import "...", @import url("..."), and @reference "..." (Tailwind CSS v4).
 */
const cssImportRE = /@(?:import|reference)\s+(?:url\(\s*)?['"]([^'"]+)['"]\s*\)?/g;

/**
 * Regex matching CSS url() references with the specifier in capture group 1.
 * Matches url('...') and url("...") but not @import/@reference url() (handled by cssImportRE).
 */
const cssUrlRE = /(?<!@(?:import|reference)\s+)url\(\s*['"]([^'"]+)['"]\s*\)/g;

/**
 * Rewrite tsconfig path aliases in CSS source text.
 * Handles @import, @reference, and url() specifiers.
...
export function resolveCssAliases(code: string, configAlias: Alias[]): string | null {
	...
	// Rewrite @import/@reference aliases
	if (result.includes('@import') || result.includes('@reference')) {
