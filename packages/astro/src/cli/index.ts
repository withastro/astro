...
const app = buildApplication(
	buildRouteMap({
		routes: {
			'create-key': buildCommand({
				async func({ verbose, silent }: { verbose?: boolean; silent?: boolean }) {
					...
					const logger = createLoggerFromFlags({ _: [], verbose, silent });
					const keyGenerator = createCryptoKeyGenerator();
					await createKey({ logger, keyGenerator });
				},
				docs: {
		...
		docs: {
			brief: 'Build faster wesbites.',
		},
	}),
	{
		name: 'astro',
		versionInfo: {
			currentVersion: `${colors.bgGreen(colors.black(` astro `))} ${colors.green(`v${ASTRO_VERSION}`)}`,
		},
	},
);
...
/** The primary CLI action */
export async function cli(argv: string[]) {
	await run(app, argv.slice(2), { process });
}
