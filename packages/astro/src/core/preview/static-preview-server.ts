...
export default async function createStaticPreviewServer(
	...
	try {
		...
		const mergedConfig = mergeConfig(userViteConfig, astroPreviewConfig);
		...
		if (
			...
			mergedConfig.preview.allowedHosts = allowedHosts;
		}

		previewServer = await preview(mergedConfig);
	} catch (err) {
