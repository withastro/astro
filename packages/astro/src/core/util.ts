...
import { AUTOGENERATED_TYPES_FILENAME } from '../content/consts.js';
...
export function resolvePagesDirectory(config: AstroConfig) {
	return new URL('./pages', config.srcDir);
}

export function resolveDotAstroDirectory(config: AstroConfig) {
	return new URL('.astro/', config.root);
}

export function resolveDotAstroTypesFile(config: AstroConfig) {
	return new URL(AUTOGENERATED_TYPES_FILENAME, resolveDotAstroDirectory(config));
}

export function resolveAstroPackageDirectory() {
	return new URL('../../', import.meta.url);
}

export function resolveEnvDtsFile(config: AstroConfig) {
	return new URL('env.d.ts', config.srcDir);
}

function isInPagesDir(file: URL, config: AstroConfig): boolean {
	const pagesDir = resolvePagesDirectory(config);
