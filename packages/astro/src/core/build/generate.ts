...
function createPlannedGeneratedPaths(
...
function shouldSkipIncrementalPath(
	pathname: string,
	route: RouteData,
	incrementalRenderPlan: ReturnType<typeof planIncrementalPageGeneration> | undefined,
	options: StaticBuildOptions,
): boolean {
	if (!incrementalRenderPlan) {
		return false;
...
async function deleteIncrementalOutputs(outputs: string[]) {
	for (const output of outputs) {
		await nodeFs.promises.rm(new URL(output), { force: true });
	}
}

function recordIncrementalReusedPath(
	pathname: string,
	route: RouteData,
	options: StaticBuildOptions,
	internals: BuildInternals,
...
function hasPublicConflict(outFile: URL, settings: AstroSettings): boolean {
	const outRoot =
		settings.buildOutput === 'static' && !settings.adapter?.adapterFeatures?.preserveBuildClientDir
			? settings.config.outDir
			: settings.config.build.client;
