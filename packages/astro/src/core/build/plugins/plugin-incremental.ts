 ...
 * the full preprocessor output (including Sass partials, `additionalData`,
 * etc.). If no compiled CSS was captured, the source file is read from disk
 * as a last resort.
 */
function hashModules(
	...
	for (const id of sortedIds) {
		...
		if (code != null && code.length > 0) {
			...
			// prerender bundle is empty. Use the compiled CSS captured during the
			// transform hook, which includes resolved preprocessor imports (Sass
			// partials, Less includes, etc.) and `additionalData` (#17974). Fall
			// back to reading the source file from disk (#17704).
...
export function pluginIncremental(internals: BuildInternals, root: URL): VitePlugin {
	// Compiled CSS captured during transform, keyed by module id. Vite's CSS
	// plugin compiles preprocessor files (Sass, Less, etc.) into plain CSS
	// during transform, resolving partials and `additionalData`. The prerender
	// bundle then strips the CSS code (it is extracted into separate assets).
	// Capturing the compiled output here lets `hashModules` hash the full
	// preprocessor result instead of just the raw entry file on disk.
