...
// Chrome (121) accepts custom-idents as generated by cssesc, but it just ignores them if they contain escaped 7-bit ASCII characters like  "\ " or "\$".
function fix(s: string) {
	let result = '';
	for (let i = 0; i < s.length; ++i) {
		result += (s[i] !== '\\' || s[i + 1].match(/[\dA-F]/))
			? s[i] : '_' + s.charCodeAt(++i).toString(16)
	}
	return result;
}
...
export function renderTransition(
	...
	const name = fix(transitionName ? cssesc(transitionName, { isIdentifier: true }) : scope);
