...
import { convertMatchToLocation, generatePaginateFunction, validateGetStaticPathsResult } from './util.js';
  ...
  try {
    ...
    if (!routeMatch.path) {
      ...
      if (!matchedStaticPath) {
        return { statusCode: 404, error: new Error(`[getStaticPaths] route pattern matched, but no matching static path not found. (${reqPath})`) };
      }
