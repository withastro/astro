...
3. **Remove stale changesets.** Delete the identified files using their exact paths from the diff output:

   ```bash
   git rm <path-to-stale-changeset>
   ```

4. **Verify.** Check that the remaining changeset `.md` files look valid (proper frontmatter with package names and bump types).

5. Do NOT commit or run `pnpm install` — the orchestrator will handle that.
...
- Do NOT remove the `.changeset/pre/` folder.
