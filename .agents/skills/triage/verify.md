...
- **Astro Compiler source** — The `withastro/compiler` repo MAY be cloned at `../compiler` (a sibling of the Astro repo root). If it exists, treat it as in-scope when researching intent. Some behaviors originate in the compiler — check `../compiler` for comments, explicit handling, and git blame when the issue involves HTML parsing, `.astro` file transformation, or compiler output.
