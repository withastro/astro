...
> "Ainutkaan ihminen ei ole saari. Web-komponenttien taas tulisi olla."

Saariarkkitehtuurin konsepti tulee [Jason Milleriltä](https://twitter.com/_developit), [Preactin](https://preactjs.com/) luojalta ja Googlen kehittäjäsuhteiden insinööriltä (DevRel Engineer).

Kesällä 2020 hän onnistui muotoilemaan ajatuksensa siitä mitä web-arkkitehtuurin tulisi olla idyllisimmässä mielessään ja kertoi tästä [blogiviestissään](https://jasonformat.com/islands-architecture/).

Tämä pohjustava viesti rajaa ja käy lävitse yleisen konseptin "saarista" arkkitehtuurisena prosessina, jota voisi käyttää web-kehittämisessä tarjoten parempaa sivustojen suorituskykyä, hakukoneoptimointia, käyttökokemusta ja kaikkea muuta. Hänen selityksensä tämän uuden paradigman kuvaukselle oli erittäin ytimekäs:

> "Perusajatus _"saarien"_ arkkitehtuurissa on petollisen yksinkertainen: muodosta HTML-sivut palvelimella ja sisällytä mukaan osoitettuja paikkoja erittäin dynaamisia alueita varten. Nämä paikat sisältävät palvelimen tuottaman HTML:n omasta piensovelluksestaan (widget). Nämä osoittavat alueet jotka voidaan elävöittää (hydrate) selaimessa erillisinä itsenäisinä piensovelluksinaan kierrättäen palvelimella tuotetun pohja-HTML:n." -Jason Miller

Meidän kannattaa ottaa katsaus taustoihin ymmärtääksemme paremmin mitä Jason tarkoittaa ehdotuksessaan ennen kuin selitämme "saariarkkitehtuurin" ja kuinka se on toteutettu Astrossa olennaisimpana kantavana tekijänä.

## Ohjelmointiparadigmat

Kuvittele yksinkertainen nettisivu. Siltä löytyy monia erilaisia sivustolla useampaan kertaan esiintyviä komponentteja, toiset komponentit sisältävät kiinteää sisältöä, jotkin taas ovat selkeämmin kehittyneitä esittäen useampia tiloja saattaen hakea tietonsa useammista ulkoisista lähteistä.
...
Ottaen huomioon [tunnettujen mallien](https://en.wikipedia.org/wiki/List_of_software_architecture_styles_and_patterns) määrän on kullekin sovellukselle oikean arkkitehtuurin valinta usein vaikeasti hankittavissa olevan kokemuksen takana.

Webkehittäjät suuntautuvat tyypillisesti kohden hyvin koeteltuja käytäntöjä ja näistä mikään ei osu tarpeeseen paremmin kohdalleen kuin [Model-View-Controller](https://en.wikipedia.org/wiki/Model%E2%80%93view%E2%80%93controller) eli **MVC**-malli.

Tässä **Model** sisältää tietorakenteen ja logiikan, joka ohjaa tiedonkäyttöä ohjelmassa. **Views** edustaa käyttäjän näkemää visuaalista näkymää, ja **Controller** yhdistää näkymät vaatimaansa tietoon _Modelista_ pohjautuen käyttäjän toimintaan.

Tämä malli toimii hyvin [asiakas-palvelin](https://en.wikipedia.org/wiki/Client%E2%80%93server_model) -pohjaisissa sovelluksissa. Kun tieto on _palvelimilla_ ja näkymät ovat usein staattisia _dokumentteja_, kulkeutuvat web-kehittäjien luomat ohjastavat palaset staattisten tiedostojen ohessa _skriptien_ muodossa.
